...
class Ion_auth_model extends CI_Model
{
	...
	public function activate($id, $code = false)
	{
		...
		if ($code !== FALSE)
		{
			...
			$this->trigger_events('extra_where');
			$db->update($this->tables['users'], $data, array($this->identity_column => $identity));
		}
		else
		{
			$data = array(
			...
			$this->trigger_events('extra_where');
			$db->update($this->tables['users'], $data, array('id' => $id));
		}
	...
	public function clear_forgotten_password_code($code) {
		...
		if ($db->count_all_results($this->tables['users']) > 0)
		{
			$data = array(
			...
			$db->update($this->tables['users'], $data, array('forgotten_password_code' => $code));
	...
	$db = $this->load->database('default', TRUE);	
		...
		$query = $db->select('id, password, salt')
	...
	$db = $this->load->database('default', TRUE);	
		...
		return $db->where($this->identity_column, $identity)
		                ->count_all_results($this->tables['users']) > 0;
	}
	...
	$db = $this->load->database('default', TRUE);	
		...
		$db->update($this->tables['users'], $update, array($this->identity_column => $identity));
	...
	public function register($username, $password, $email, $additional_data = array(), $groups = array())
	{
		...
		$db->insert($this->tables['users'], $user_data);
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $db->select($this->identity_column . ', username, email, id, password, active, last_login')
		                  ->where($this->identity_column, $db->escape_str($identity))
		                  ->limit(1)
		                  ->get($this->tables['users']);
	...
	function get_attempts_num($identity)
	{
		...
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			if (strlen($identity) > 0) $db->or_where('login', $identity);

			$qres = $db->get($this->tables['login_attempts']);
	...
	public function increase_login_attempts($identity) {
		...
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			$ip_address = $this->_prepare_ip($this->input->ip_address());
			return $db->insert($this->tables['login_attempts'], array('ip_address' => $ip_address, 'login' => $identity, 'time' => time()));
		}
		return FALSE;
	}
	...
	public function clear_login_attempts($identity, $expire_period = 86400) {
		...
		if ($this->config->item('track_login_attempts', 'ion_auth')) {
			...
			$db->where(array('ip_address' => $ip_address, 'login' => $identity));
			// Purge obsolete login attempts
			$db->or_where('time <', time() - $expire_period, FALSE);
	...
	public function users($groups = NULL)
	{
		...
		if (isset($groups))
		{
			...
			if (isset($groups) && !empty($groups))
			{
				...
				$db->where_in($this->tables['users_groups'].'.group_id', $groups);
			}
		}
		...
		{
			$db->limit($this->_ion_limit, $this->_ion_offset);
		...
		{
			$db->order_by($this->_ion_order_by, $this->_ion_order);

			$this->_ion_order    = NULL;
			$this->_ion_order_by = NULL;
		}

		$this->response = $db->get($this->tables['users']);
	...
	public function get_users_groups($id=FALSE)
	{
		...
		return $db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as id, '.$this->tables['groups'].'.name, '.$this->tables['groups'].'.description')
	...
	public function add_to_group($group_id, $user_id=false)
	{
		...
		return $db->insert($this->tables['users_groups'], array( $this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id));
	}
	...
	public function remove_from_group($group_ids=false, $user_id=false)
	{
		...
		if( ! empty($group_ids))
		{
			if(is_array($group_ids))
			{
				...
				{
					$db->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_id, $this->join['users'] => (int)$user_id));
				}
			...
			{
				return $db->delete($this->tables['users_groups'], array($this->join['groups'] => (int)$group_ids, $this->join['users'] => (int)$user_id));
			}
		}
		// otherwise remove user from all groups
		else
		{
			return $db->delete($this->tables['users_groups'], array($this->join['users'] => (int)$user_id));
		}
	}
	...
	public function groups()
	{
		...
		$this->response = $db->get($this->tables['groups']);
	...
	public function group($id = NULL)
	{
		...
		if (isset($id))
		{
			$db->where($this->tables['groups'].'.id', $id);
		}
	...
	public function update($id, array $data)
	{
		...
		$this->trigger_events('extra_where');
		$db->update($this->tables['users'], $data, array('id' => $user->id));
	...
	**/
	public function delete_user($id)
	{
		...
		$db->delete($this->tables['users'], array('id' => $id));
	...
	public function update_last_login($id)
	{
		...
		$db->update($this->tables['users'], array('last_login' => time()), array('id' => $id));
	...
	public function remember_user($id)
	{
		...
		$db->update($this->tables['users'], array('remember_code' => $salt), array('id' => $id));
